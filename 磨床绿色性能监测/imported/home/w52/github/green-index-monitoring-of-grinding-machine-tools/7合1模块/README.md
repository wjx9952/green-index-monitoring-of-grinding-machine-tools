...
程序会自动列出 `/dev/serial*`、`/dev/ttyUSB*`、`/dev/ttyACM*`、`/dev/ttyAMA*` 和 `/dev/ttyS*`，使用规格书规定的 `9600 8N1` 读取数据。当前树莓派默认优先连接 UART1 的 `/dev/ttyAMA1`。

UART1 接线：

- 传感器 TX → 树莓派 GPIO1 / 物理 Pin 28（RXD1）
- 传感器 RX → 树莓派 GPIO0 / 物理 Pin 27（TXD1）
- 传感器 GND → 树莓派 GND
